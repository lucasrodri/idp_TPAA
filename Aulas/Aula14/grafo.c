...
void alg_prim(Grafo *gr, int orig, int *pai)
{
    ...
    while (1)
    {
        ...
        for (i = 0; i < NV; i++)
        {
            ...
            if (pai[i] != -1)
            {
                ...
                {
                    // procura menor peso
                    // achou vertice vizinho não visitado
                    if(pai[gr->arestas[i][j]] == -1) {
                        if (primeiro)
                        { // procura aresta de menor caminho
...
void alg_kruskal(Grafo *gr, int orig, int *pai) {
    int i, j, dest, primeiro;
    int NV = gr->nro_vertices;
    float menor_peso;
    ...
    while(1) {
        ...
        for (i = 0; i < NV; i++) { //percorre todos os vértices
            for (j = 0; j < gr->grau[i]; j++) { //arestas do vértice i
